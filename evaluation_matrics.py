...
class Method(object):
    ...
    def _cafa_go_aupr(self, labels, preds):
        ...
        for t in range(1, 100):
            ...
            for i in range(0, n):
                ...
                num_pred = len(pred_gos)
                num_true = len(prot2goterms[i])
                num_overlap = len(prot2goterms[i].intersection(pred_gos))  # 求交集的数量
        ...
        max_F = np.max(F_list)
        max_index = np.argmax(F_list)
        thresh_at_max_F = thresh_list[max_index]
        print("Max_F：", max_F)
        print("Best_threshold：", thresh_at_max_F)
    ...
    def smin(self, keep_pidx=None):
        ...
        for i, t in enumerate(thresholds):
            ss[i] = normalizedSemanticDistance(Y_true, (Y_pred >= t).astype(int), self.termIC, avg=True,
                                               returnRuMi=False)
