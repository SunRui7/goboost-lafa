...
def load_GO_annot(filename):
    ...
    with open(filename, mode='r') as tsvfile:
        ...
        next(reader, None)  # skip the headers
        counts = {ont: np.zeros(len(goterms[ont]), dtype=float) for ont in onts}  # 这个字典用于存储每个GO术语的计数。
        for row in reader:
            prot, prot_goterms = row[0], row[1:]
            # row[1:]是指从1开始的后面的每一列元素，row[1]对应蛋白质mf的多种功能标签，row[2]对应bp，row[3]对应cc.
            prot2annot[prot] = {ont: [] for ont in onts}
            for i in range(3):
                goterm_indices = [goterms[onts[i]].index(goterm) for goterm in prot_goterms[i].split(',') if goterm != '']
                # 首先将prot_goterms[i]按照逗号分割，然后对于每一个非空的GO术语，找到它在goterms[onts[i]]中的索引，将这些索引存储在列表goterm_indices中。
                prot2annot[prot][onts[i]] = np.zeros(len(goterms[onts[i]]))  # 初始化标签全为0
                prot2annot[prot][onts[i]][goterm_indices] = 1.0
                counts[onts[i]][goterm_indices] += 1.0  # 将正标签设为1
