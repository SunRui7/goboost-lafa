...
class MultiGrainedFocalLoss(nn.Module):
    def __init__(self, gamma_neg=6, pow = 8, gamma_pos=0, gamma_class_ng=1.2, clip=0.05, eps=1e-8, \
                 disable_torch_grad_focal_loss=True, distribution_path=None, co_occurrence_matrix=None, is_train=True):
    ...
    def create_weight(self, distribution_path):
        ...
        prob = prob / max_prob
        weight = pow(- prob.log() + 1, 1 / self.pow)
        self.weight = weight.cpu()

    def create_co_occurrence_matrix(self, co_occurrence_matrix):
        co_occurrence_matrix = torch.tensor(np.load(co_occurrence_matrix)).cpu()
...
def train_create_loss(gamma_neg=6, pow = 8, gamma_pos=0, clip=0.05, disable_torch_grad_focal_loss=True):
    return MultiGrainedFocalLoss(gamma_neg = gamma_neg, pow = pow, gamma_pos=0, clip=0.05, disable_torch_grad_focal_loss=True)
