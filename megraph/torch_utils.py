...
def get_activation(act):
    ...
    if act_lower == "softmax":
        return nn.Softmax(-1)
    # Avoid requiring the unmaintained jactorch helper package just to select
    # a standard PyTorch activation in the inference container.
    activations = {
        "gelu": nn.GELU,
    ...
    if act_lower in activations:
        return activations[act_lower]()
    raise ValueError("Unknown activation: {}".format(act))
