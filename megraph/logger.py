...
# Modified based on stable-baselines
# https://github.com/Stable-Baselines-Team/stable-baselines/blob/master/stable_baselines/logger.py
...
class Logger:
    ...
    def set_folder_and_formats(
        self, folder: Optional[str], output_formats: List[KVWriter]
    ):
        self.dir = folder
...
def configure(
    ...
    logger.set_folder_and_formats(folder, output_formats)
...
def read_json(filename: str) -> pandas.DataFrame:
    ...
    if pandas is None:
        raise ImportError("pandas is required to read logger JSON files")
...
def read_csv(filename: str) -> pandas.DataFrame:
    ...
    """
    if pandas is None:
        raise ImportError("pandas is required to read logger CSV files")
...
def dump_params(logdir, params):
    dump_params = params.copy()
    for k, v in dump_params.items():
        dump_params[k] = str(v)
        print(f"{k}: {v}")
    with open(os.path.join(logdir, "params.json"), "wt") as f:
